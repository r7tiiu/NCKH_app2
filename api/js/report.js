/*
==================================================
StressCheck – report.js (có AI + lưu kết quả)
==================================================
*/
...
function getAdvice(id, risk) {
    const advice = {
        ...
        peer: "Hãy nhớ rằng mỗi người có một thế mạnh riêng. Tập trung vào mục tiêu của bản thân thay vì so sánh với người khác.",
...
// ==================================================
// LƯU KẾT QUẢ LÊN SERVER (mới thêm)
// Chỉ lưu: overall stress, lĩnh vực stress cao nhất, % của 6 lĩnh vực
// ==================================================
async function saveResultToServer() {
    ...
    // % của 6 lĩnh vực (id → title mapping)
    const domains = {};
    rank.forEach(item => {
        const risk = item.risk !== undefined ? item.risk : (item.score || 0);
    ...
    // Nếu chạy local (file://, localhost) thì không gọi API để tránh lỗi
    const isLocal = window.location.protocol === 'file:' ||
                    window.location.hostname === 'localhost' ||
                    window.location.hostname === '127.0.0.1';
    if (isLocal) {
        console.log('🔧 Local dev – bỏ qua lưu kết quả. Payload:', {
            overallStress, highestDomain, domains
        });
        return;
    }
...
async function buildReport() {
    try {
        ...
        // ⬇️ Lưu kết quả SAU khi mọi thứ đã có (không chặn UI)
        saveResultToServer();
    } catch (error) {
        console.error("Build report failed", error);
        showResultsPage();
        // Vẫn cố lưu nếu tính điểm đã xong
        saveResultToServer();
    } finally {
...
console.log('✅ report.js loaded (có AI + lưu kết quả)');